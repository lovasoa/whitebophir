...
- Stored SVG parsing is intentionally tiered:
  ...
  - Streamed stored items expose a buffer-backed element object. Only `id` is read eagerly; attribute/content/path helpers decode lazily when higher-level code actually needs that data, and clean rewrite items must continue to write their original raw buffer slice without materializing full raw item text.
