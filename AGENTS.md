...
- Live socket/runtime mutations use numeric `type` codes from [mutation types](./client-data/js/mutation_type.js); stored SVG keeps string tag names such as `rect`, `path`, and `text` because that is document syntax, not the runtime protocol.
