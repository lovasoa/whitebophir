...
- Canonical item shape keeps scalar attrs in `attrs`, stores `transform` only once at the top level, and isolates pencil child arrays only when ownership changes (copy/public escape/full-write materialization). Text content length is payload state, not geometry: text admission/index bounds are the stable anchor/font-size box so no text edit can become a geometry rewrite or truncation.
