...
- Page shell that server-renders the toolbar and loads the module entrypoint for the board runtime: [board document](./client-data/board.html), [board module boot](./client-data/js/board_main.js). For stored `.svg` boards, the board document now streams the authoritative SVG baseline straight through the HTML response so the browser can render it progressively as bytes arrive, without materializing the full SVG in server JS; the head also provides `modulepreload` hints for the board runtime graph plus the visible board tool modules and `Cursor`, while the async board module entrypoint waits only for the streamed `#canvas` root before importing the runtime and starting the normal boot sequence. Board HTML, canonical SVG, and preview HTTP responses negotiate built-in Node compression from `Accept-Encoding`, preferring `zstd` when both sides support it, then `br`, then `gzip`. The board chrome (`#loadingMessage`, HUD, menu, JSON payloads, early chrome bootstrap, module entrypoint) must stay before the streamed board markup in the HTML so the visible UI can paint and react before the SVG finishes streaming. Legacy `.json` fallback boards still render a generated inline SVG. Board boot publishes explicit DOM phases on `document.documentElement.dataset.boardPhase` and dispatches `wbo:board-phase` events, and initial URL-hash viewport restore happens before socket connection startup.
...
- The client opens Socket.IO with handshake query `board=<boardName>` plus tool/color/size metadata as soon as the board runtime is initialized and the viewport hash has been restored; the first connect reuses the streamed inline baseline already in the page and asks the server only for seq replay, while reconnects refresh the authoritative SVG baseline first. The server reads the user secret from the cookie before emitting `boardstate` plus the authoritative snapshot `broadcast`, while visible tool modules and `Cursor` are already being discovered from the document head via `modulepreload`.
