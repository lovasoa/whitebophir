...
- [server configuration](./server/configuration.mjs) exports values parsed from `process.env` at module load with no memoization layer or reset hook, and also exposes a pure `parseConfigurationFromEnv(env)` helper for tests that need an explicit env snapshot. Anything on the hot path must not re-read env per item or per coordinate.
