...
- The client opens Socket.IO with handshake query `board=<boardName>` plus tool/color/size metadata after critical tools are booted, and the server reads the user secret from the cookie before emitting `boardstate` plus the authoritative snapshot `broadcast`; non-critical replay-safe tools continue booting in parallel with that connection.
