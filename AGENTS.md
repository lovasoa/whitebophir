...
- [server configuration](./server/configuration.mjs) is evaluated from `process.env` at module load with no memoization layer or reset hook. Tests that change env must re-import config consumers after clearing the relevant module cache or using an ESM cache-bust query. Anything on the hot path must not re-read env per item or per coordinate.
