...
- [server configuration](./server/configuration.mjs) exports values parsed from `process.env` at module load with no memoization layer or reset hook. Tests that need an explicit env snapshot use the shared parsers in [configuration values](./server/configuration_values.mjs). Anything on the hot path must not re-read env per item or per coordinate.
