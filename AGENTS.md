...
- Client state machine + staged tool boot + send/receive plumbing: [board runtime](./client-data/js/board.js). Board viewport panning, zooming, scale math, wheel/pinch ownership, and URL hash sync are centralized in [viewport controller](./client-data/js/board_viewport.js); gesture handlers there must stay O(1) and never traverse or measure board SVG contents.
