...
AntiZoomAndPan.prototype.init=function(){var svgRoot=svg;this.svgNodes=[];this.x_trans=0;this.y_trans=0;this.scale=1;this.lastTM=svgRoot.createSVGMatrix();svgRoot.addEventListener('SVGZoom',this,false);svgRoot.addEventListener('SVGScroll',this,false);svgRoot.addEventListener('SVGResize',this,false);};
...
Mouser.prototype.init=function(){this.svgNode=null;this.handles=[];this.shapes=[];this.lastPoint=null;this.currentNode=null;this.realize();};
...
Mouser.prototype.unregisterAll=function(){for(var i=0;i<this.handles.length;i++){this.handles[i].select(false);}this.handles=[];};
Mouser.prototype.unregisterShapes=function(){for(var i=0;i<this.shapes.length;i++){var shape=this.shapes[i];shape.select(false);shape.selectHandles(false);shape.showHandles(false);shape.unregisterHandles();}this.shapes=[];};
...
Mouser.prototype.mousemove=function(e,x,y){var svgPoint=this.getUserCoordinate(this.currentNode,x,y);var newPoint=new Point2D(svgPoint.x,svgPoint.y);var delta=newPoint.subtract(this.lastPoint);var updates=[];var updateId=new Date().getTime();this.lastPoint.setFromPoint(newPoint);for(var i=0;i<this.handles.length;i++){var handle=this.handles[i];var owner=handle.owner;handle.translate(delta);if(owner!=null){if(owner.lastUpdate!==updateId){owner.lastUpdate=updateId;updates.push(owner);}}else{updates.push(handle);}}for(var i=0; i<updates.length; i++){updates[i].update();}};
...
Intersection.prototype.init=function(status){this.status=status;this.points=[];};
...
Polynomial.prototype.init=function(coefs){this.coefs=[];for(var i=coefs.length-1;i>=0;i--)this.coefs.push(coefs[i]);this._variable="t";this._s=0;};
...
Polynomial.prototype.toString=function(){var coefs=[];var signs=[];for(var i=this.coefs.length-1;i>=0;i--){var value=Math.round(this.coefs[i]*1000)/1000;if(value!==0){var sign=(value<0)?" - ":" + ";value=Math.abs(value);if(i>0)if(value===1)value=this._variable;else value+=this._variable;if(i>1)value+="^"+i;signs.push(sign);coefs.push(value);}}signs[0]=(signs[0]===" + ")?"":"-";var result="";for(var i=0; i<coefs.length; i++)result+=signs[i]+coefs[i];return result;};
...
Path.prototype.refresh=function(){var d=[];for(var i=0;i<this.segments.length;i++){d.push(this.segments[i].toString());}this.svgNode.setAttributeNS(null,"d",d.join(" "));};
...
AbsolutePathSegment.prototype.init=function(command,params,owner,previous){this.command=command;this.owner=owner;this.previous=previous;this.handles=[];var index=0;while(index<params.length){var handle=new Handle(params[index],params[index+1],owner);this.handles.push(handle);index+=2;}};
...
AbsolutePathSegment.prototype.toString=function(){var points=[];var command="";if(this.previous==null||this.previous.constructor!==this.constuctor)command=this.command;for(var i=0; i<this.handles.length; i++){points.push(this.handles[i].point.toString());}return command+points.join(" ");};
...
AbsoluteArcPath.prototype.init=function(command,params,owner,previous){var point=[];var y=params.pop();var x=params.pop();point.push(x,y);AbsoluteArcPath.superclass.init.call(this,command,point,owner,previous);this.rx=parseFloat(params.shift());this.ry=parseFloat(params.shift());this.angle=parseFloat(params.shift());this.arcFlag=parseFloat(params.shift());this.sweepFlag=parseFloat(params.shift());};
AbsoluteArcPath.prototype.toString=function(){var points=[];var command="";if(this.previous.constructor!==this.constuctor)command=this.command;return command+[this.rx,this.ry,this.angle,this.arcFlag,this.sweepFlag,this.handles[0].point.toString()].join(",");};
...
AbsoluteHLineto.prototype.init=function(command,params,owner,previous){var prevPoint=previous.getLastPoint();var point=[];point.push(params.pop(),prevPoint.y);AbsoluteHLineto.superclass.init.call(this,command,point,owner,previous);};
AbsoluteHLineto.prototype.toString=function(){var points=[];var command="";if(this.previous.constructor!==this.constuctor)command=this.command;return command+this.handles[0].point.x;};
...
AbsoluteLineto.prototype.toString=function(){var points=[];var command="";if(this.previous.constructor!==this.constuctor)if(this.previous.constructor!==AbsoluteMoveto)command=this.command;return command+this.handles[0].point.toString();};
...
RelativeLineto.prototype.toString=function(){var points=[];var command="";var lastPoint;var point;if(this.previous)lastPoint=this.previous.getLastPoint();else lastPoint=new Point(0,0);point=this.handles[0].point.subtract(lastPoint);if(this.previous.constructor!==this.constuctor)if(this.previous.constructor!==RelativeMoveto)cmd=this.command;return cmd+point.toString();};
...
Polygon.prototype.init=function(svgNode){if(svgNode.localName==="polygon"){Polygon.superclass.init.call(this,svgNode);var points=svgNode.getAttributeNS(null,"points").split(/[\s,]+/);this.handles=[];for(var i=0; i<points.length; i+=2){var x=parseFloat(points[i]);var y=parseFloat(points[i+1]);this.handles.push(new Handle(x,y,this));}}else{throw new Error("Polygon.init: Invalid SVG Node: "+svgNode.localName);}};
...
Polygon.prototype.refresh=function(){var points=[];for(var i=0;i<this.handles.length;i++){points.push(this.handles[i].point.toString());}this.svgNode.setAttributeNS(null,"points",points.join(" "));};
...
Polygon.prototype.getIntersectionParams=function(){var points=[];for(var i=0;i<this.handles.length;i++){points.push(this.handles[i].point);}return new IntersectionParams("Polygon",[points]);};
