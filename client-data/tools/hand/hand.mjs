...
(function hand() {
  ...
  blockedSelectionButtons.forEach((buttonIndex) => {
   ...
   * @param {ToolPointerEvent} evt
   */
  function startScalingTransform(_x, _y, evt) {
    evt.preventDefault();
    hideSelectionButtons();
    selectorState = selectorStates.transform;
   ...
   * @param {ToolPointerEvent} evt
   */
  function releaseSelector(_x, _y, _evt) {
   ...
   * @param {boolean} force
   */
  function moveSelector(x, y, _evt, force) {
    if (selectorState === selectorStates.selecting) {
   ...
   */
  function startHand(_x, _y, evt, isTouchEvent) {
    if (!isTouchEvent) {
      selected = {
   ...
   * @param {boolean} isTouchEvent
   */
  function moveHand(_x, _y, evt, isTouchEvent) {
  ...
  function isSelectorActive() {
    return !!handTool.secondary?.active;
  }
