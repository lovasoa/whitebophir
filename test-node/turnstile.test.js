...
test("server-side Turnstile token validation rejects hostname and action mismatches", async function () {
  await withEnv(
    {
      TURNSTILE_SECRET_KEY: "test-secret",
      TURNSTILE_SITE_KEY: "test-site-key",
    ...
    async function () {
      ...
      try {
        ...
        globalThis.fetch = async function actionMismatch() {
          return {
            json: async () => ({
              success: true,
              hostname: "board.example",
        ...
        let actionMismatchAck = null;
        await tokenHandler("valid-token", (result) => {
          actionMismatchAck = result;
        });
        assert.deepEqual(actionMismatchAck, { success: false });
        assert.equal(
          socket.turnstileValidatedUntil,
          undefined,
          "action mismatch should not validate the socket",
        );
      } finally {
        globalThis.fetch = originalFetch;
      }
