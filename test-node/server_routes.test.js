...
test("cookie-authorized moderator board responses are private and vary by cookie", async () => {
  const dirs = await createServerDirs();
  const moderatorSecret = "abcdefabcdefabcdefabcdefabcdefab";
  await fs.writeFile(
    boardSvgFile(dirs.historyDir, "cookie-moderated"),
    '<svg id="canvas" xmlns="http://www.w3.org/2000/svg" version="1.1" width="5000" height="5000" data-wbo-format="whitebophir-svg-v2" data-wbo-seq="5" data-wbo-readonly="false"><defs id="defs"></defs><g id="drawingArea"></g><g id="cursors"></g></svg>',
    "utf8",
  );
  ...
  const app = await createTestServer(
    createServerConfig(dirs, {
      ...
      BOARD_MODERATORS: {
        "cookie-moderated": new Set([moderatorSecret]),
      },
      WEBROOT: CLIENT_WEBROOT,
      IS_DEVELOPMENT: false,
    }),
  ...
  try {
    ...
    const pageResponse = await request(
      app,
      "/boards/cookie-moderated",
      headers,
    );
    const svgResponse = await request(
      app,
      "/boards/cookie-moderated.svg",
      headers,
    );
    const cachedSvgResponse = await request(
      app,
      "/boards/cookie-moderated.svg",
      {
        ...headers,
        "If-None-Match": String(svgResponse.headers.etag),
      },
    );

    assert.equal(pageResponse.statusCode, 200);
    assert.equal(
      pageResponse.headers["cache-control"],
      "private, max-age=0, must-revalidate",
    );
    assert.match(String(pageResponse.headers.vary), /Cookie/);
    assert.equal(svgResponse.statusCode, 200);
    assert.equal(
      svgResponse.headers["cache-control"],
      "private, max-age=0, must-revalidate",
    );
    assert.match(String(svgResponse.headers.vary), /Cookie/);
    assert.equal(cachedSvgResponse.statusCode, 304);
    assert.equal(
      cachedSvgResponse.headers["cache-control"],
      "private, max-age=0, must-revalidate",
    );
    assert.match(String(cachedSvgResponse.headers.vary), /Cookie/);
  } finally {
    await closeServer(app);
  }
});
